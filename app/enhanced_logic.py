...
async def transcribe_browser_audio(audio_data: bytes, use_local_whisper=True):
    """Transcribe audio data sent from browser
    
    Args:
        audio_data: Raw audio bytes from browser (WebM format)
        use_local_whisper: Whether to use local Faster Whisper (default: True for free operation)
    ...
    try:
        print(f"Received {len(audio_data)} bytes of audio from browser")
        ...
        print(f"Saved audio to {temp_webm}")
        
        # Convert webm to wav using ffmpeg (required for Whisper)
        ...
        # Transcribe using local Whisper or OpenAI
        transcription = None
        
        if use_local_whisper:
            # Use local Faster Whisper (FREE!)
            from .transcription import transcribe_with_whisper
            print("Using local Faster Whisper for transcription...")
            transcription = transcribe_with_whisper(temp_wav)
        else:
            # Use OpenAI API (costs money)
            from .transcription import transcribe_with_openai_api
            print(f"Using OpenAI API for transcription (model: {enhanced_transcription_model})...")
            transcription = await transcribe_with_openai_api(temp_wav, enhanced_transcription_model)
        ...
        while enhanced_conversation_active:
            try:
                ...
                if character_name.startswith("story_") or character_name.startswith("game_"):
                    ...
                    if len(local_conversation_history) > 6:
                        local_conversation_history = local_conversation_history[-6:]
...
import requests
import os
#from .shared import DEBUG, NEON_GREEN, RESET_COLOR
from app.shared import DEBUG, NEON_GREEN, RESET_COLOR

OLLAMA_BASE_URL = os.getenv("OLLAMA_BASE_URL", "http://172.17.0.1:11434")
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "llama3.2:latest")
MAX_CHAR_LENGTH = 4000  # fallback if not defined elsewhere

async def enhanced_chat_completion_ollama(prompt: str, system_message: str, mood_prompt: str, conversation_history=None):
    #
    #Replacement for enhanced_chat_completion using Ollama API.
    #Returns the AI response text.
    #

    try:
        # Combine system + mood prompts
        combined_system_message = f"{system_message}\n{mood_prompt}".strip()

        # Build prompt with conversation history
        full_prompt = combined_system_message + "\n"
        ...
        payload = {
            "model": OLLAMA_MODEL,
            "prompt": full_prompt,
            "max_tokens": min(MAX_CHAR_LENGTH, 1024),
            "temperature": 0.8
        }

        # Send POST request in a thread (requests is blocking)
        ...
        data = response.json()
        ai_text = data.get("choices", [{}])[0].get("text", "").strip()

        if DEBUG:
            print(NEON_GREEN + ai_text + RESET_COLOR)

        return ai_text

    except Exception as e:
        print(f"Error in enhanced_chat_completion_ollama: {e}")
