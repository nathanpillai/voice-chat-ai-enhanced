document.addEventListener("DOMContentLoaded", function () {
    ...
    function connectWebSocket() {
        ...
        // FIXED: Use correct protocol and remove port 8000
        const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        websocket = new WebSocket(`${protocol}//${window.location.host}/ws_enhanced`);
        ...
        fetch('/start_enhanced_conversation', {
            ...
            .then(data => {
                ...
                if (data.status === 'success') {
                    displayMessage("Conversation started. Speak now!", "system-message");
                } else {
                    displayMessage("Failed to start conversation: " + data.message, "error-message");
    ...
    stopBtn.addEventListener('click', function () {
        ...
        fetch('/stop_enhanced_conversation', {
            ...
            .then(data => {
                ...
                hasStarted = false;
                if (data.status === 'success') {
                    displayMessage("Conversation stopped.", "system-message");
                }
            })
            .catch(error => {
