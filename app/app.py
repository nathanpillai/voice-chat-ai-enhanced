...
def init_espeak_tts_voice(voice_name):
    global ESPEAK_TTS_VOICE
    ESPEAK_TTS_VOICE = voice_name
    print(f"Switched to eSpeak TTS voice: {voice_name}")

def init_coqui_tts_voice(voice_name):
    global COQUI_TTS_VOICE
    COQUI_TTS_VOICE = voice_name
    print(f"Switched to Coqui TTS voice: {voice_name}")
...
async def process_and_play(prompt, audio_file_pth):
    ...
    if TTS_PROVIDER == 'openai':
        ...
        output_path = os.path.join(output_dir, 'output.wav')
        success = await espeak_text_to_speech(prompt, output_path)
        if success and os.path.exists(output_path):
            print("Playing generated audio...")
            await send_message_to_clients(json.dumps({"action": "ai_start_speaking"}))
            await play_audio(output_path)
            ...
            print("Failed to generate eSpeak audio.")
        else:
            print("Error: eSpeak audio file not found after generation.")
            await send_message_to_clients(json.dumps({
                "action": "error",
                "message": "eSpeak audio file not found after generation"
            }))  

    elif TTS_PROVIDER == 'coqui':
        output_path = os.path.join(output_dir, 'output.wav')
        success = await coqui_text_to_speech(prompt, output_path)
        if success and os.path.exists(output_path):
            print("Playing generated audio...")
            await send_message_to_clients(json.dumps({"action": "ai_start_speaking"}))
            await play_audio(output_path)
            ...
            print("Failed to generate Coqui audio.")
        else:
            print("Error: Coqui audio file not found after generation.")
            await send_message_to_clients(json.dumps({
                "action": "error",
                "message": "Coqui audio file not found after generation"
            }))  
...
def chatgpt_streamed(user_input, system_message, mood_prompt, conversation_history):
    ...
    if MODEL_PROVIDER == 'ollama':
        ...
        try:
            print(f"Debug: Sending request to Ollama: {OLLAMA_BASE_URL}/api/chat")
            response = requests.post(f'{OLLAMA_BASE_URL}/api/chat', headers=headers, json=payload, stream=True, timeout=30)
            response.raise_for_status()
            ...
            for line in response.iter_lines(decode_unicode=True):
                if line:
                    try:
                        ...
                        # Ollama API format: {"message": {"content": "..."}, "done": false}
...
async def espeak_text_to_speech(text, output_path):
    """Convert text to speech using eSpeak TTS (local, free)."""
    try:
        import subprocess
        import asyncio
        
        # Get voice speed from environment (espeak uses words-per-minute, default 175)
        voice_speed = os.getenv("VOICE_SPEED", "1.0")
        # Convert speed multiplier to WPM (175 is default)
        wpm = int(175 * float(voice_speed))
        
        # Get voice name (default to en-us+f3)
        espeak_voice = os.getenv("ESPEAK_TTS_VOICE", "en-us+f3")
        
        # Build espeak command
        # -w writes to file, -s sets speed in words per minute, -v sets voice
        command = [
            "espeak-ng",
            "-v", espeak_voice,
            "-s", str(wpm),
        ...
        # Run espeak command asynchronously
        ...
        if process.returncode == 0:
            print(f"eSpeak TTS generated successfully: {output_path}")
            return True
        else:
            error_msg = stderr.decode() if stderr else "Unknown error"
            print(f"eSpeak TTS error: {error_msg}")
            await send_message_to_clients(json.dumps({
                "action": "error",
                "message": f"eSpeak TTS error: {error_msg}"
            }))
            return False
            
    except FileNotFoundError:
        print("eSpeak not found. Please install espeak-ng: apt-get install espeak-ng")
        await send_message_to_clients(json.dumps({
            "action": "error",
            "message": "eSpeak not installed on server"
        }))
        return False
    except Exception as e:
        print(f"Error in eSpeak TTS: {e}")
        await send_message_to_clients(json.dumps({
            "action": "error",
            "message": f"eSpeak TTS error: {str(e)}"
        }))
        return False


async def coqui_text_to_speech(text, output_path):
    """Convert text to speech using Coqui TTS (local, free)."""
    try:
        import asyncio
        ...
        # Get voice/model name
        coqui_voice = os.getenv("COQUI_TTS_VOICE", "tts_models/en/ljspeech/tacotron2-DDC")
        
        # Initialize TTS model if not already done
        global tts
        if tts is None or not hasattr(tts, 'model_name') or tts.model_name != coqui_voice:
            print(f"Loading Coqui TTS model: {coqui_voice}")
            os.environ["COQUI_TOS_AGREED"] = "1"  # Auto-agree to terms
            
            # Load model in a thread to avoid blocking
            def load_tts():
                return TTS(coqui_voice).to(device)
        ...
        # Generate speech
        def generate():
            # Check if model supports speed parameter
            if hasattr(tts, 'synthesizer') and hasattr(tts.synthesizer, 'tts_config'):
                # XTTS-like models
                wav = tts.tts(text=text, speed=voice_speed)
            else:
                # Basic models (no speed control)
                wav = tts.tts(text=text)
            
            # Apply speed adjustment manually if needed and model doesn't support it
            if voice_speed != 1.0 and not (hasattr(tts, 'synthesizer') and hasattr(tts.synthesizer, 'tts_config')):
                from scipy import signal
                import numpy as np
                # Resample to adjust speed
                samples = len(wav)
                new_samples = int(samples / voice_speed)
                wav = signal.resample(wav, new_samples)
            
            return wav
        
        wav = await asyncio.to_thread(generate)
        
        # Get sample rate
        if hasattr(tts, 'synthesizer') and hasattr(tts.synthesizer, 'tts_config'):
            sample_rate = tts.synthesizer.tts_config.audio.get("sample_rate", 22050)
        else:
            sample_rate = 22050  # Default sample rate
        
        # Save to file
        sf.write(output_path, wav, sample_rate)
        print(f"Coqui TTS generated successfully: {output_path}")
        return True
        ...
        await send_message_to_clients(json.dumps({
            "action": "error",
            "message": f"Coqui TTS error: {str(e)}"
        }))
        return False
